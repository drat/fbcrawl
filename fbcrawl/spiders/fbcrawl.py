...
class FacebookSpider(scrapy.Spider):
    ...
    name = 'fb'
    custom_settings = {
        'FEED_EXPORT_FIELDS': ['name','source','profile_img','shared_from','date','text','images', \
    ...
    def __init__(self, *args, **kwargs):
        ...
        if 'page' in kwargs:
            if self.page.find('/groups/') != -1:
                ...
                self.group = 0
            # self.page += "?sorting_setting=CHRONOLOGICAL"
...
#        open_in_browser(response)
        ...
        for post in response.xpath("//div[contains(@data-ft,'top_level_post_id')]"):     
            ...
            if current_date is None:
                ...
                try:
                    ...
                    date_month = month.index(date[1]) + 1
                    current_date = datetime(2019,int(date_month),int(date[0]))
                date = str(date)
    ...
    def parse_post(self,response):
        ...
        shared_from = response.xpath("//td/div/h3/text()").get()
        print("shared_from: {}".format(shared_from))
        if shared_from is not None:
            shared_url = response.xpath("//td/div/h3/a/@href").get()
            print("shared_url: {}".format(shared_url))
            new._add_value("shared_from", shared_url)
     #   new.add_xpath('date','//div/div/abbr/text()')
        new.add_xpath('text','//div[@data-ft]//p//text() | //div[@data-ft]/div[@class]/div[@class]/text()')
        
        new.add_xpath('images','//div[@data-ft]/div/a/img/@src')
